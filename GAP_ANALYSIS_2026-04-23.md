...
- Backend: `ementas-api/src`
- Frontend: `ementas-app/src`
...
- Docker Compose end-to-end SIAC validation (real course/semester):
  - stack: `docker compose up -d --build` in `ementas-api`;
...
- Validation evidence:
  - `ementas-api`: `jest ComponentDocumentFlow.spec.ts --runInBand` passed.
...
- Backend infrastructure stabilization published in commit `68a429d`:
  - explicit TypeScript module resolution alignment in `ementas-api`;
...
#### Evidence
The resulting work was published in two conventional commits with separate intent and validation scope: `68a429d` for backend tooling stability and `901435a` for frontend governance/publication behavior. Validation evidence included successful execution of `npm run typecheck` in `ementas-api`, plus `npm run typecheck` and `npm run test:run` in `ementas-app`, with passing tests covering publication approval and user management flows.
...
| --- | --- | --- | --- |
| Development environment stability for TypeScript dependency resolution | `68a429d` | `ementas-api`: `npm run typecheck` | `ementas-api/tsconfig.json` |
...
- Use `ementas-docs/README.md` as the executive summary source for delivered increments.
...
| --- | --- | --- | --- | --- |
| E01US01 Generate invite link | Partial | `ementas-api/src/routers/UserInviteRouter.ts` route `/generate` has no auth/admin guard; `ementas-app/src/pages/UserListPage/index.tsx` uses this action | Any user (or anonymous) may issue invite links | Add `ensureAuthenticated` + role guard for invite generation; add forbidden tests |
| E01US02 Register from invite | Implemented | `ementas-api/src/routers/UserRouter.ts` POST `/:inviteToken`; `ementas-app/src/routes.tsx` `/cadastrar/:inviteToken` | Token misuse risk if validation rules are weak | Add integration tests for expired/invalid/reused token |
...
| E01US04 List teachers | Implemented | `ementas-api/src/routers/UserRouter.ts` GET `/` guarded by admin middleware | Frontend route is only authenticated, not explicitly admin-gated | Add UI guard by role to hide/deny user management page for non-admin |
| E02US01 Login | Implemented | `ementas-api/src/routers/AuthRouter.ts` POST `/login`; auth context in `ementas-app/src/contexts/auth/index.tsx` | Low | Keep as is; add regression tests after dependency upgrades |
| E02US02 Reset password | Implemented | `ementas-api/src/routers/AuthRouter.ts` POST `/reset-password`; `ementas-app/src/pages/FogotPasswordPage/index.tsx` | Naming typo in page folder can hurt maintainability | Rename `FogotPasswordPage` -> `ForgotPasswordPage` in a safe refactor slice |
| E03US01 Create discipline/content | Implemented | `ementas-api/src/routers/ComponentRouter.ts` POST `/`; DTO validation and workload model present | Medium data consistency risk in workload copy/update | Add integration test for create + draft mirror consistency |
| E03US02 Search/list disciplines | Implemented | `ementas-api/src/controllers/ComponentController.ts` getComponents supports search/pagination; list UI in `ementas-app/src/pages/ComponentListPage/index.tsx` | Autocomplete behavior may differ from requirement wording | Validate UX acceptance and add tests for search by code/name |
...
| E03US04 Official change history | Implemented | `GET /components/:id/logs` in `ementas-api/src/routers/ComponentRouter.ts`; API client in `ementas-app/src/api/component/index.ts` | Log filtering semantics need contract tests | Add tests for chronological order and type filtering |
...
| E03US07 Export discipline | Implemented | `GET /components/:id/export` returns PDF in `ementas-api/src/controllers/ComponentController.ts` | Endpoint is public by design, review policy | Decide if public export is expected; if not, add auth |
| E03US08 Crawler import | Implemented | `POST /components/import` in `ementas-api/src/routers/ComponentRouter.ts` with auth | High cost/abuse risk if not role-restricted | Restrict import to admin and add rate/timeout safeguards |

## Critical Risks (Priority Order)
1. Invite generation endpoint without authentication/authorization.
2. User deletion endpoint appears to require only authentication, not admin role (`ementas-api/src/routers/UserRouter.ts` DELETE `/:id`).
